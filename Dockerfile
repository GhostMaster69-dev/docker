...
RUN apt-get update && \
    yes | unminimize && \
    DEBIAN_FRONTEND=noninteractive apt-get install -y apt-utils autoconf automake bash-completion bc binutils-dev bison build-essential bzip2 bzr ca-certificates ccache clang clangd clang-format clang-tidy clang-tools cmake curl default-jdk dialog dirmngr dpkg-dev emacs-nox file fish flex gawk g++ gcc gdb gh git git-lfs gnupg gperf help2man htop imagemagick iputils-ping jq less libbz2-dev libc6-dev libcurl4-openssl-dev libdb-dev libdebuginfod-dev libedit-dev libelf-dev libevent-dev libffi-dev libfl-dev libgdbm-dev libglib2.0-dev libgmp-dev libjpeg-dev libkrb5-dev liblzma-dev libmagickcore-dev libmagickwand-dev libmaxminddb-dev libmysqlclient-dev libncurses5-dev libncursesw5-dev libpng-dev libpq-dev libreadline-dev libsdl1.2-dev libsqlite3-dev libssl-dev libtool libwebp-dev libxml2-dev libxmlsec1-dev libxslt-dev libyaml-dev liblzma-dev lld lldb llvm locales lsb-core lsb-release lsof lz4 lzma lzop make man-db maven mercurial multitail nano ncftp ninja-build netbase openssh-client parallel patch patchelf pngcrush rclone ripgrep rlwrap schedtool screen screenfetch software-properties-common squashfs-tools ssl-cert stow subversion sudo tar texinfo time tk-dev tmate tmux tzdata u-boot-tools vim w3m wget whiptail xsltproc xvfb xz-utils zip zlib1g-dev zsh zstd && \
...
RUN echo 'sudo apt-get update && sudo apt-get upgrade -y && sudo apt-get autoremove -y' >> .bashrc
